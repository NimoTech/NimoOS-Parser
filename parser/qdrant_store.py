...
class QdrantStore:
    ...
    def count_vectors(self) -> dict:
        ...
        "visual" counts photo captions (kind == caption, one point per asset,
        see pipeline_visual) with an exact filtered count over the indexed
        `kind` field — small by construction. "text" is everything else in
        text_chunks (document chunks, and legacy points without a kind),
        derived as the collection's approximate total minus the captions so a
        /stats poll never scans the whole collection. visual_chunks is a
        never-written placeholder collection and is deliberately not
        consulted — counting it reported 0 forever while the captions
        inflated the text total.
        """
        caption = qm.FieldCondition(key="kind", match=qm.MatchValue(value="caption"))
        captions = self.client.count(
            self.text_collection, count_filter=qm.Filter(must=[caption]),
            exact=True).count
        total = self.client.count(self.text_collection, exact=False).count
        return {"text": max(total - captions, 0), "visual": captions}
