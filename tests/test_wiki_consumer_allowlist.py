...
def test_allowed_ext_returns_index(conn):
    ...
    assert _op_for_event(ev, conn=conn) == "index"
...
def test_disallowed_ext_returns_none(conn):
    for path in ("/x.MOV", "/x.mp4", "/x.jpg", "/x.zip", "/x.immich", "/x.sql.gz"):
        ...
        assert _op_for_event(ev, conn=conn) is None, f"should skip {path}"


def test_delete_event_passes_through_for_any_ext():
    # delete 任何文件都要 forward 到 parser (用于清向量), no conn needed
...
def test_unknown_ext_skipped(conn):
    ...
    assert _op_for_event(ev, conn=conn) is None
