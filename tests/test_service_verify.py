...
from parser.repo_state import get_verify_last
from parser.service_verify import RootDiff, VerifyRunner, apply_root_diff, diff_root, run_verify
...
class FakeWiki:
    def __init__(self, roots, files, fail_roots=False):
        self.roots, self.files, self.fail_roots = roots, files, fail_roots
    async def list_roots(self):
        if self.fail_roots:
            raise RuntimeError("wiki down")
        return [{"id": r, "path": "/" + r, "enabled": True} for r in self.roots]
...
async def test_run_verify_retires_unknown_roots_and_reports_counts(conn):
    ...
    assert res["ok"] is True and res["trigger"] == "manual" and res["finished_at"] == 50
...
async def test_verify_runner_is_single_flight(conn):
    ...
    runner = VerifyRunner(conn, MagicMock(), SlowWiki(roots=[], files={}), MagicMock())
